...
export interface MuscleGainPlan {
  id: string;
  name: string;
  description: string;
  trainingTime: string;
...
export const muscleGainPlans: MuscleGainPlan[] = [
  {
    id: 'morning_early',
    name: '早饭后练（早起版）',
...
export function getMuscleGainPlan(planId: string): MuscleGainPlan | undefined {
  return muscleGainPlans.find(plan => plan.id === planId);
}

export function getAllMuscleGainPlans(): MuscleGainPlan[] {
  return muscleGainPlans;
}
...
// 过滤空的餐次配置
export function filterEmptyMeals(plan: MuscleGainPlan): MuscleGainPlan {
...
// 获取过滤后的计划
export function getFilteredMuscleGainPlan(planId: string): MuscleGainPlan | undefined {
  const plan = getMuscleGainPlan(planId);
  return plan ? filterEmptyMeals(plan) : undefined;
}

// 获取所有过滤后的计划
export function getAllFilteredMuscleGainPlans(): MuscleGainPlan[] {
  return muscleGainPlans.map(plan => filterEmptyMeals(plan));
}
