<template>
  <div class="muscle-movements-page">
    <div class="container mx-auto px-4 py-8">
      ...
      <!-- 图片网格展示 -->
      <div class="movement-diagrams-grid">
        <div 
          v-for="(movement, index) in muscleMovements" 
          :key="index"
          class="movement-card"
        >
      ...
      <div v-if="showImageViewer" class="image-viewer-modal" @click="closeImageViewer">
        <div class="modal-content" @click.stop>
          ...
          <div class="next-button" @click="nextImage" v-if="currentImageIndex < muscleMovements.length - 1">
            <i class="iconfont icon-arrow-right"></i>
        </div>
          <img 
            :src="muscleMovements[currentImageIndex].image" 
            class="modal-image"
          >
          <div class="image-counter">
            {{ currentImageIndex + 1 }} / {{ muscleMovements.length }}
          </div>
        </div>
      </div>
    </div>
...
// 下一张图片
const nextImage = () => {
  if (currentImageIndex.value < muscleMovements.value.length - 1) {
    currentImageIndex.value++;
  }
};
</script>
